...
async def start_handler(client: Client, msg: types.Message):
    ...
    username = '@Menfessveaperbot' if not msg.from_user.username else '@' + msg.from_user.username
...
async def gagal_kirim_handler(client: Client, msg: types.Message):
    ...
    username = '@Riizzvbss' if not msg.from_user.username else '@' + msg.from_user.username
    mention = msg.from_user.mention
